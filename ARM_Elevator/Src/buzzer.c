...
unsigned int happy_birthday[] =
{
 ...
 unsigned int arrival[] =
 {
  ...
  unsigned int arrival_duration[] = {1,1,1,2};
...
void arrival_buzzer()
{
	...
	if(buzzer_mode == 1)	// elevator ==> arrival
	{
	    if(buzzer_step < 4)
	    {
			if(TIM11_10ms_buzzer_counter <= 20)
			{
				__HAL_TIM_SET_AUTORELOAD(&htim3, divide_freq / arrival[buzzer_step]);
				__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, divide_freq / arrival[buzzer_step] / 2);
			}
			else
			{
